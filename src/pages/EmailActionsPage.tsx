...
export default function EmailActionsPage() {
  ...
  const statusGroups = [
    { title: 'Draft', items: emailActions.filter((e) => e.status === 'draft') },
    { title: 'Pending', items: emailActions.filter((e) => e.status === 'pending') },
    { title: 'Sent', items: emailActions.filter((e) => e.status === 'sent') },
    { title: 'Failed', items: emailActions.filter((e) => e.status === 'failed') },
  ];

  return (
    <div className="space-y-6 max-w-6xl">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Email Actions</h1>
        <p className="text-sm text-muted-foreground mt-1">Manage email-related note actions. Delivery is handled by your backend.</p>
      </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground truncate mt-0.5">To: {ea.recipient}</p>
        ...
        <div className="hidden lg:block w-80 shrink-0">
          {selected ? (
            <div className="bg-card rounded-lg border p-5 shadow-card sticky top-20 space-y-4">
              <h3 className="font-medium text-sm">Email Action Details</h3>
                    ...
                    {['draft', 'pending', 'sent'].map((step, i) => {
                      ...
                      return (
                        <div key={step} className="flex items-center gap-2 text-xs">
                          ...
                          <span className={cn(done ? 'text-foreground' : 'text-muted-foreground', 'capitalize')}>{step}</span>
                          {selected.status === 'failed' && step === 'pending' && (
                            <span className="text-priority-urgent text-xs">→ Failed</span>
                          )}
                        </div>
                      );
                    })}
            ...
            <div className="bg-card rounded-lg border p-8 text-center shadow-card">
              ...
              <p className="text-sm text-muted-foreground">Select an email action to view details</p>
            </div>
          )}
        </div>
      </div>
